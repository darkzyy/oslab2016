...
boot_alloc(uint32_t n)
{
	...
	//
	// NOTE : I used to think char *result is useless,
	// however, when use nextfree as a global var , it makes sense
	//
...
page_init(void)
{
	...
	//     
	//  NOTE1:
	//     nextfree modified in boot_alloc index the first free page,
	//     defining it as a gloabal var is more convenient
	//  NOTE2:
	//	   To simplify, can I merely use memory above page tables ?
	//
	...
	for (i = 0; i < npages; i++) {
		...
		else if(i*PGSIZE >= PADDR(nextfree)){
			pages[i].pp_ref = 0;
			pages[i].pp_link = page_free_list;
			page_free_list = &pages[i];
		}
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if(pgdir[PDX(va)] & PTE_P){
		...
		pgte = (pte_t*)KADDR(PTE_ADDR(pgdir[PDX(va)]));
		pgte += PTX(va);
	}else if(create == 1){
		struct PageInfo *npage;
		if((npage = page_alloc(ALLOC_ZERO))){		
			npage->pp_ref++;
			pgte = (pte_t*)page2kva(npage);		  
			pgdir[PDX(va)] = PTE_P | PTE_W | PTE_U | page2pa(npage);
			pgte += PTX(va);
		}
	}
	return pgte;
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if(pgdir[PDX(va)] & PTE_P){ // present
		...
		if(page_as_pt != NULL){
			...
			pt = &pt_base[PTX(va)];
			pgdir[PDX(va)] = page2pa(page_as_pt)
			//phsical mm is used in mmu!
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
	...
	if(*pt & PTE_P){ // present
		if(PTE_ADDR(*pt) == page2pa(pp)){// inserted at the same va
			//*pt |= perm | PTE_P;
			*pt = perm | PTE_P | page2pa(pp);
			return 0;
		}
...
{
	pte_t *pt = pgdir_walk(pgdir, va, 0); // not create
	...
	/*page table entry -> page physical address -> corresponding PageInfo */
}
