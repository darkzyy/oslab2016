	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
    ...
    // locate env
    struct Env *src_env_store, *dst_env_store;
    int err = envid2env(srcenvid, &src_env_store, 1);
    if(err){
        return err;
    }
    err = envid2env(dstenvid, &dst_env_store, 1);
    if(err){
        return err;
    }

    pte_t* pte_store;
    struct PageInfo *pp = page_lookup(src_env_store->env_pgdir, srcva, &pte_store);
    if(!pp){
        return -E_INVAL;
    }
    ...
    err = page_insert(dst_env_store->env_pgdir, pp, dstva, perm);
    return err;
}
